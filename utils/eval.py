...
def si_curve(model, test_matrix, delta, mu_delta, targetreg=None):
    """This function considers the stochastic intervention curve under
    a distributional shift fromt the observed exposure g1(t|x)=g(t-d|x)"""
    dev = next(iter(model.parameters())).device

    mu_delta_hat = torch.zeros_like(mu_delta).to(dev)
    t = test_matrix[:, 0].to(dev)
    x = test_matrix[:, 1:-1].to(dev)

    for j, d in enumerate(delta):
        with torch.no_grad():
            out = model.forward(t * d, x) # apply the given t to the batch and calculate the mean
            if targetreg:
                tr_out = targetreg(t * d).data
                g = out[0].data.squeeze()
                mu_delta_hat[j] = (out[1] + tr_out / (g + 1e-6)).mean()
            else:
                mu_delta_hat[j] = out[1].mean()
    mse = F.mse_loss(mu_delta, mu_delta_hat)

    return mu_delta_hat, mse
