...
class TR(nn.Module):
    ...
    def forward(self, t):
        out = self.spb.forward(t).to(t.device)
...
class Drnet(nn.Module):
    def __init__(self, cfg_density, num_grid, cfg, isenhance, h=1, att_layers=0):
        ...
        self.Q = Multi_head(cfg, isenhance, h=h)
...
class TransPlus(nn.Module):
    def __init__(self, cfg_density, num_grid, cfg, isenhance, dropout=0.1, att_layers=1, nhead=2, class_head='mean'):
        super(TransPlus, self).__init__()
        ...
        d_model = cfg_density[0][1]
        self.treat_emb = Embeddings(d_model)
        self.dosage_emb = Embeddings(d_model)
        self.variable_emb = Embeddings(d_model)
        self.dropout = nn.Dropout(dropout)
        encoder_layers = TransformerEncoderLayer(d_model, nhead, d_model, dropout)
        self.transformer_encoder = TransformerEncoder(encoder_layers, att_layers)


        # construct the density estimator
        layer_cfg = cfg[-1]
        self.estimator = Treat_Linear(layer_cfg[0], layer_cfg[1], act=layer_cfg[3], istreat=0, isbias=layer_cfg[2],islastlayer=1)
    ...
    def forward(self, t, x):
        hidden_t = self.treat_emb(t).view(t.shape[0],1,-1)
        hidden_x = self.variable_emb(x)
        hidden = self.dropout(hidden_t.expand(hidden_x.shape) + hidden_x)
        hidden = self.transformer_encoder(hidden)

        hidden = self.pool_class_feature(hidden)
        g = None #self.density_estimator_head(t, hidden)
        t_hidden = torch.cat((torch.unsqueeze(t, 1), hidden), 1)
        Q = self.estimator(t_hidden)

        return g, Q
    
    def pool_class_feature(self, x):
        if self.class_head == 'last':
            return x[:,-1,:]
        elif self.class_head == 'first':
            return x[:,0,:]
        elif self.class_head == 'max':
            return torch.max(x, dim=1)[0].squeeze()
        elif self.class_head == 'mean':
            return torch.mean(x, dim=1).squeeze()
        elif self.class_head == 'sum':
