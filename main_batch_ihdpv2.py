...
from utils.eval import curve, si_curve
...
# criterion
def criterion(out, y, alpha=0.5, epsilon=1e-6, si=False):
    loss =  F.mse_loss(out[1].squeeze(),  y.squeeze()) # - alpha * torch.log(out[0] + epsilon).mean()
    return loss
...
if __name__ == "__main__":
    ...
    # i/o
    parser.add_argument('--data_dir', type=str, default='dataset/ihdp/ihdp_v2/tr_h_1.75_te_l_0.25_h2.0', help='dir of eval dataset')
    parser.add_argument('--save_dir', type=str, default='logs/ihdpv2/tr_h_1.75_te_l_0.25_h2.0/', help='dir to save result')
    ...
    method_list = ['Tarnet_tr', 'Drnet_tr', 'Vcnet_tr', 'TransTEE_tr']
    ...
    for model_name in method_list:
        ...
        elif model_name == 'TransTEE' or model_name == 'TransTEE_tr':
            num_cov=25
            num_t=1
            num_heads = 2
            att_layers = 1
        ...
        if model_name.endswith('_tr'):
            isTargetReg = 1
        else:
            isTargetReg = 0

        # best cfg for each model
        if 'Tarnet' in model_name:
            init_lr = 0.05
            tr_init_lr = 0.001
            alpha = 1.0
        ...
        elif 'Vcnet' in model_name:
            init_lr = 0.0005
            tr_init_lr = 0.001
            alpha = 0.5

            Result['Vcnet'] = []
        elif 'VcnetAtt' in model_name:
            init_lr = 0.0005
            tr_init_lr = 0.001
            alpha = 0.5
    ...
    if args.plt_adrf:
        ...
        for i, (key, val) in enumerate(grid.items()):
            x = val[0, :]
            y = val[1, :]
            plt.scatter(x, y, marker='h', label=key[0], alpha=0.5, color=colors[i], s=15)
            print('x=', x, '\ny=', y)
