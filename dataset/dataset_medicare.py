...
class DataMedicare:
    ...
    def init(self):
        ...
        train_data = {
            "treatment": torch.FloatTensor(treatment_train.values).squeeze(1),
            "outcome": torch.FloatTensor(outcome_train.values).squeeze(1),
            "covariates": torch.FloatTensor(covariates_train.values),
            "offset": torch.FloatTensor(offset_train.values).squeeze(1),
        }
        test_data = {
            "treatment": torch.FloatTensor(treatment_test.values).squeeze(1),
            "outcome": torch.FloatTensor(outcome_test.values).squeeze(1),
            "covariates": torch.FloatTensor(covariates_test.values),
            "offset": torch.FloatTensor(offset_test.values).squeeze(1),
        }
